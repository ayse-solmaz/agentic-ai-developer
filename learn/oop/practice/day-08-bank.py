...
#payable protocol (contract):
# get_balance() -> number
# deposit(amount) -> none / true
# withdraw(amount) -> none / true
class Customer:
...
class BankAccount:
    ...
    def close_with_transfer(self,other):
        """tüm bakiyeyi diğer hesaba taşı"""
        amount=self._balance
        if amount > 0:
            self.transfer_to(other,amount)      
...
wallet = Wallet(500)
pay(wallet, 100)   # Wallet
pay(acc1, 50)      # BankAccount
acc2_before = acc2.get_balance()
acc1.close_with_transfer(acc2)
print(acc1.get_balance())  # 0 olmalı
print(acc2.get_balance())  # önceki + acc1'in kalanı